...
```
REMOTEVOICE/
├── relay/      云中继（Node + TS）：配对/房间 + 豆包ASR + LLM后处理
...
- 手机 PWA（前端，HTTPS）：https://remotevoice-pwa.sunlinhua.workers.dev — Cloudflare Worker 静态资产托管，配置见 [`wrangler.jsonc`](wrangler.jsonc)，重新部署 `npx wrangler deploy`。
- ⚠️ **中继尚未公网部署**：PWA 默认连同源 `wss://.../ws`，但 Worker 只托管静态前端，没有中继。要让线上 PWA 真正能用，需把 `relay/`（Node 常驻 + 密钥）部署到一台有公网 wss 的机器（见下方一键脚本），再到 PWA「设置」里把中继地址填成它的 `wss://` 地址。本机临时联调可用 Cloudflare Tunnel（cloudflared）把 localhost:8787 暴露成临时 wss。

完整部署方案见 [`deploy/`](deploy/)：一键脚本、nginx 配置、systemd 服务、PM2 配置、详细文档。
...
- **真实中文语音验证**：ASR 协议已用真实凭证连通豆包（无任何 decode/sequence 错误），但合成波形无法产出可读中文，需用手机录真人语音验证识别效果。
- ~~**`ARK_API_KEY`**~~ ✅ 已配置并验证：LLM 后处理在线，四条样例加标点/顿号/纠错均正常（`npm run e2e:llm`）。
- **公网服务器 + 域名**（用于异地访问；本地调试用 `http://localhost` 即可）。

> **注（doubao-seed-2.0 是推理模型）**：后处理调用必须带 `thinking:{type:"disabled"}`（已在 `relay/src/doubao-llm.ts` 写死）。否则补标点这种琐事也会触发思维链，实测延迟 30~50s，必然撞 8s 超时回退原文。换非推理模型时可去掉该字段。本地验证三脚本：`npm run e2e:pair` / `e2e:asr` / `e2e:llm`。
